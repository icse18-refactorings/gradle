...
For `api` configuration introduced by `java-library` plugin, please consult <<java_library_separation, Java Library Plugin>> and <<dependency_management_for_java_projects, Dependency Management Tutorial>>.
...
Annotation processors need to opt-in to this feature, otherwise they will result in full recompilation.
As a user you can see which annotation processors are causing full recompilations in the `--info` log.
...
Processors that don't fit these categories will result in full recompilation as before.

As a processor author, please consult the information below to decide which category your processor falls in.
Then add a file called

`META-INF/gradle/incremental.annotation.processors`

and add one line for each of your incremental processors and their category, separated by a comma.

    org.example.MyProcessor,ISOLATED
    org.example.MyOtherProcessor,AGGREGATING

===== Isolated annotation processors
These look at each annotated element in isolation, creating generated files or validation messages for it.
For instance a `RepositoryProcessor` could create a repository implementation for each interface annotated with `@Repository`


Isolated processors have the following limitations:
...
These aggregate several source files into one ore more output files or validation message.
For example, a `ServiceRegistryProcessor` might create a single `ServiceRegistry` with one method for each type annotated with `@Service`
...
Gradle will always recompile all files that contain annotations which the aggregating processor registered itself for.
Gradle will also always recompile all files generated by an aggregating processor.
In other words, aggregating processors create small non-incremental clusters in the code base.
...
Projects with large test suites can take a long time to execute even though a failure occurred early on leading to unnecessary wait times (especially on CI). To short circuit this behavior, the api:org.gradle.api.tasks.testing.Test#getFailFast()[`failFast`] property allows you to cause the `test` task to fail after the first test failure instead of running all tests. When this property is true, the resulting output will only show the results of tests that have completed up to and including the failure. To enable this fail fast behavior in your build file, set the `failFast` property to `true`:
...
[NOTE]
====
With http://junit.org/junit5/docs/current/user-guide[JUnit Platform], only `includes`/`excludes` is used to filter test classes, `scanForTestClasses` has no effect.
====
...
Properties files are used in many places during Java development and so Gradle makes it easy to create them as a normal part of the build. Just use the api:org.gradle.api.tasks.WriteProperties[] task.
